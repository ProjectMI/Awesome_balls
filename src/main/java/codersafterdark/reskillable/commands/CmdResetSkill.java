...
public class CmdResetSkill extends CommandBase
{
    ...
    public void execute(@Nonnull final MinecraftServer server, @Nonnull final ICommandSender sender, @Nonnull final String[] args) throws CommandException {
        ...
        if (!MinecraftForge.EVENT_BUS.post((Event)new LevelUpEvent.Pre((EntityPlayer)player, skill, 1, oldLevel))) {
            skillInfo.setLevel(1);
            skillInfo.respec();
            data.saveAndSync();
            MinecraftForge.EVENT_BUS.post((Event)new LevelUpEvent.Post((EntityPlayer)player, skill, 1, oldLevel));
            ToastHelper.sendSkillToast(player, skill, 1);
            sender.sendMessage((ITextComponent)new TextComponentTranslation("reskillable.command.success.resetskill", new Object[] { skillName, player.getDisplayName() }));
        }
        else {
            sender.sendMessage((ITextComponent)new TextComponentTranslation("reskillable.command.fail.resetskill", new Object[] { skillName, player.getDisplayName() }));
        }
    }
